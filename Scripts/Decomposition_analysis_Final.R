...
#import data
decomp <- read_csv(meta)%>%
  filter(Units == "prop. decom" & 
           Trait != "Leaf-piercer")%>% ##not enough leaf piercers in data set
           drop_na(Std.dev.control)

#Calculate log Response Ratio
decomp.es<-escalc(measure="ROM",
            m1i= Mean.control, m2i=Mean.experimental,
            sd1i=Std.dev.control, sd2i=Std.dev.experimental,
            n1i=SS.control, n2i= SS.experimental, 
            data=decomp,
